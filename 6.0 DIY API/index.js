...
app.get("/random",(req, res) => {
  let randomNo = Math.floor(Math.random() * jokes.length);
  let randomJoke = jokes[randomNo];
  res.json(randomJoke)
})
//2. GET a specific joke
app.get("/jokes", (req, res) => {
  let jokeId = req.query.id;
  res.json(jokes.find(joke => joke.id == jokeId));
});
//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  let jokeType = req.query.type;
  let selectedJokes = jokes.filter(joke => joke.jokeType === jokeType)
...
app.post("/jokes", (req, res) => {
  ...
  let updatedObject = {["id"]: newId, ...req.body}
  jokes = jokes.concat(updatedObject);
  // console.log(jokes.length);
  if(jokes.length == newId){
    res.status(200).send(jokes[newId - 1]);
  }else{
...
app.put("/jokes", (req, res) => {
  ...
  res.status(200).json(jokes[jokeId - 1]);
